...
const PlaceInfoView = (props) => {
    ...
    const getLatLongInFormat = (inputCoordinate) => {
        ...
        let coordinateFormat = userPref?.appSettings?.coordinateFormat ??
            CoordinateFormat.DecDeg;
        ...
        switch (placeType) {
            ...
            case PlaceType.City: {
                ...
                placeDetailsObj = {
                    ...
                    //     value: currentPlaceItem.stateItem?.latitude
                    // },
                    // stateLongitude: {
                    //     type: 'stateLongitude',
                    //     title: 'State Longitude',
                    //     value: currentPlaceItem.stateItem?.longitude
                    // },
                }
    ...
    const renderPlaceInfo = () => {
        return (
            <Accordion
                ...
                allowMultiple
                defaultIndex={[0]}>
                {isSearchPlaceSectionWithinSettings(SearchPlaceSectionType.InputCoordinates) &&
                                ...
                                <AccordionPanel>
                                    <Box
                                        ...
                                        {Object.keys(state?.placeDetailsObj).map((item, index) => {
                                            return renderPlaceProperty(state?.placeDetailsObj[item], `place${index}`)
                                        })}
                                    </Box>
                                </AccordionPanel>
                            </AccordionItem>}
                                ...
                                >
                                    <Box
                                        ...
                                        {Object.keys(state?.countryDetailsObj).map((item, index) => {
                                            return renderPlaceProperty(state?.countryDetailsObj[item], `country${index}`)
                                        })}
                                    </Box>
                                </AccordionPanel>
                            </AccordionItem>}
                                                ...
                                                <Box
                                                    ...
                                                    <Box
                                                        ...
                                                        {Object.keys(timezoneObj).map((item, timeIndex) => {
                                                            return renderTimeZoneProperty(timezoneObj[item], timeIndex)
                                                        })}
                                                    </Box>
